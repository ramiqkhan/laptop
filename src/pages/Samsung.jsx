  ...
  useEffect(() => {
    const fetchSamsungProducts = async () => {
      ...
      try {
        ...
        // Filter out items strictly belonging to Samsung brand
        const samsungOnly = Array.isArray(data) ? data.filter(p => p.brand?.toUpperCase() === 'SAMSUNG') : [];
          ...
          <main className="flex-1">
            <div className="bg-white rounded-2xl border border-[#E6E6E6] p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
              <p className="text-gray-700 font-semibold text-xs sm:text-sm">
                Showing {filteredAndSortedProducts.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredAndSortedProducts.length)} of {filteredAndSortedProducts.length} Samsung Laptops
              </p>
            ...
            {currentProducts.length === 0 ? (
              <div className="text-center py-12 text-gray-500 font-bold bg-white rounded-2xl border border-[#E6E6E6]">
                No Samsung laptops matching your configuration found.
              </div>
            ) : (
