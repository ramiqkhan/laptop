  ...
  useEffect(() => {
    const fetchLenovoProducts = async () => {
      ...
      try {
        ...
        // Filter out items strictly belonging to Lenovo brand
        const lenovoOnly = Array.isArray(data) ? data.filter(p => p.brand?.toUpperCase() === 'LENOVO') : [];
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <nav className="hidden sm:block text-xs md:text-sm text-gray-500 font-bold uppercase tracking-widest">
            ...
            <span className="text-gray-800">Lenovo</span>
          </nav>
        </div>
          ...
          <main className="flex-1">
            <div className="bg-white rounded-2xl border border-[#E6E6E6] p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
              <p className="text-gray-700 font-semibold text-xs sm:text-sm">
                Showing {filteredAndSortedProducts.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredAndSortedProducts.length)} of {filteredAndSortedProducts.length} Lenovo Laptops
              </p>
            ...
            {currentProducts.length === 0 ? (
              <div className="text-center py-12 text-gray-500 font-bold bg-white rounded-2xl border border-[#E6E6E6]">
                No Lenovo laptops matching your configuration found.
              </div>
            ) : (
