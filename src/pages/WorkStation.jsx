    ...
    try {
      ...
      if (existingItem) {
        ...
        currentCart.push({
          ...
          img: productImage,
          quantity: 1,
          brand: product.brand || "workstations",
...
useEffect(() => {
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Changed from 'workstations' to 'workstation' to match database schema
      let url = `${BASE_URL}/api/products?category=workstation`;
      if (searchQuery) {
...
const filteredAndSortedProducts = useMemo(() => {
    // Changed from 'workstations' to 'workstation'
    let result = products.filter(p => p.category === 'workstation');
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-xl md:text-3xl font-black text-black italic uppercase tracking-tighter">
            {selectedProduct ? 'Extreme Performance' : 'Workstations'}
          </h1>
          ...
          <main className="flex-1">
            {selectedProduct ? (
              ...
              <div className="animate-in fade-in duration-500">
                ...
                >
                  <ArrowLeft size={18} /> Back to Workstations
                </button>
              ...
              <>
                <div className="bg-white rounded-2xl border border-[#E6E6E6] p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
                  <p className="text-gray-700 font-semibold text-xs sm:text-sm">
                    Showing {filteredAndSortedProducts.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredAndSortedProducts.length)} of {filteredAndSortedProducts.length} Workstations
                  </p>
