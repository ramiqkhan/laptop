...
const LaptopPage = () => {
    ...
    try {
      ...
      if (existingItem) {
        ...
        currentCart.push({
          ...
          img: productImage,
          quantity: 1,
          brand: product.brand || "Laptop Premium",
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        // Build the query parameter cleanly using standard URL API definitions
        const query = new URLSearchParams();
        
        if (selectedFilters.Brand.length > 0) {
          query.append('brand', selectedFilters.Brand[0]);
        }
        if (searchQuery) {
          query.append('search', searchQuery);
        }

        const url = `${BASE_URL}/api/products${query.toString() ? `?${query.toString()}` : ''}`;
    ...
    fetchProducts();
  }, [location.search, selectedFilters.Brand]); // Fixed dependency array
  ...
  // --- FILTER & SORT USEMEMO (FIXED) ---
  const filteredAndSortedProducts = useMemo(() => {
    // Note: Adjusted category verification out if backend returns something else. 
    // Restore 'p.category === "products"' explicitly if you verified it matching backend payloads.
    let result = [...products]; 
          ...
          <main className="flex-1">
            <div className="bg-white rounded-2xl border border-[#E6E6E6] p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
              <p className="text-gray-700 font-semibold text-xs sm:text-sm">
                Showing {filteredAndSortedProducts.length > 0 ? indexOfFirstItem + 1 : 0}-{Math.min(indexOfLastItem, filteredAndSortedProducts.length)} of {filteredAndSortedProducts.length} Laptops
              </p>
            ...
            {currentProducts.length === 0 ? (
              <div className="text-center py-12 text-gray-500 font-bold bg-white rounded-2xl border border-[#E6E6E6]">
                No laptops matching your configuration found.
              </div>
            ) : (
